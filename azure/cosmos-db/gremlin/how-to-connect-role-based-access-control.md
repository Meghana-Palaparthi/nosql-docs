...
1. Then, list all the role definitions associated with your Azure Cosmos DB for Gremlin account by using `az rest`. Finally, review the output and locate the role definition named `Cosmos DB Built-in Data Contributor`. The output contains the unique identifier of the role definition in the `id` property. Record this value because you need to use it in the assignment step later in this article.
